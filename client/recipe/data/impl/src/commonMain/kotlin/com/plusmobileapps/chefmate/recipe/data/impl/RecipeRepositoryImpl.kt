...
class RecipeRepositoryImpl(
    ...
    override fun getRecipes(): Flow<List<Recipe>> = db.getAll()
        .asFlow()
        .map { it.executeAsList() }
        .map { it.map { item -> item.toRecipe() } }
        .flowOn(ioContext)

    override suspend fun createRecipe(recipe: Recipe): Recipe = withContext(ioContext) {
    ...
    override suspend fun updateRecipe(recipe: Recipe): Recipe = withContext(ioContext) {
        db.transactionWithResult {
            db.update(
                id = recipe.id,
                title = recipe.title,
